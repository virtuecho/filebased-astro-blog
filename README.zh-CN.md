...
src/content/posts/      Markdown 文章文件 — 每篇文章在自己的目录中
src/content/posts/{postId}/  每篇文章是一个目录：index.md + 附件
...
一篇正式文章对应一个目录，包含 Markdown 文件和所有附件：
...
`pnpm dev` 用于日常编辑。`pnpm build` 用于检查并生成 `dist/`。`pnpm preview` 用于预览已经生成的 `dist/`。
...
```text
1. 生成随机 postId UUID
2. 创建 src/content/posts/{postId}/ 目录
3. 创建 src/content/posts/{postId}/index.md 并填入草稿 frontmatter
```
...
6. 文件会复制到 `src/content/posts/{postId}/`，和 `index.md` 放在一起。
...
Markdown 路径里不要写 `public`。正文中的图片路径相对于文章目录。
...
`postId` 是随机 UUID。创建文章时生成。它用于命名存放文章和所有附件的目录。
...
```text
src/content/posts/{postId}/  文章目录 — Markdown、图片和所有附件都在同一个目录里
public/images/site/          全站图片（头图、背景图等）
```

文章图片、下载文件、截图和封面图都在文章目录中，和 `index.md` 放在一起。

站点全局图片（背景图、头图）放在 `public/images/site/`：
...
Git hooks 会尽量保持快速，只处理暂存文件。完整的 `pnpm check` 仍然是推荐的
提交前验证命令。
...
pnpm new-post      # 新建草稿文章和附件目录
pnpm edit-post     # 打开 Markdown 文章
