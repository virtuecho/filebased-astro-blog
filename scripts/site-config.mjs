...
function usage() {
  console.log(`Usage:
  pnpm site-config                        interactive menu
  pnpm site-config -- --show              print current settings
  pnpm site-config -- --lang zh-CN        set default language
  pnpm site-config -- --bg-header URL     set header background image
  pnpm site-config -- --bg-body URL       set body background image
  pnpm site-config -- --bg-site URL       set site background image
  pnpm site-config -- --font-family FONT  set body font family
  `);
  exit(0);
}
...
async function interactiveMenu(settings) {
  ...
  console.log(
    `10. Sidebar About Title (en): ${enSidebar.aboutTitle || '(default)'}`,
  );
  console.log(
    `11. Sidebar About Text (en) : ${enSidebar.aboutText || '(default)'}`,
  );
  console.log(
    `12. Sidebar About Title (zh): ${zhSidebar.aboutTitle || '(default)'}`,
  );
  console.log(
    `13. Sidebar About Text (zh) : ${zhSidebar.aboutText || '(default)'}`,
  );
  console.log(`14. Admin Intro (en)  : ${enAdmin.intro || '(default)'}`);
  console.log(`15. Admin Intro (zh)  : ${zhAdmin.intro || '(default)'}`);
  console.log(
    `16. Body BG Image     : ${theme.bodyBackgroundImage || '(none)'}`,
  );
  console.log(
    `17. Header BG Image   : ${theme.headerBackgroundImage || '(none)'}`,
  );
  console.log(
    `18. Site BG Image     : ${theme.siteBackgroundImage || '(none)'}`,
  );
  console.log(
    `19. Header Text Color   : ${theme.headerTextColor || '(default)'}`,
  );
  console.log(
    `20. Header Desc Color   : ${theme.headerDescriptionColor || '(default)'}`,
  );
  console.log(
    `21. Header Min Height   : ${theme.headerMinHeight || '(default)'}`,
  );
  ...
  switch (num) {
    ...
    case 5:
      ...
      if (!settings.copy['zh-CN'])
        settings.copy['zh-CN'] = {
          site: { title: '', description: '', footer: '' },
        };
    ...
    case 12:
      ...
      if (settings.copy['zh-CN'])
        settings.copy['zh-CN'].sidebar.aboutTitle = value;
      break;
    case 13:
      value = await ask('Sidebar About Text (zh-CN)', zhSidebar.aboutText);
      if (settings.copy['zh-CN'])
        settings.copy['zh-CN'].sidebar.aboutText = value;
      break;
    case 14:
    ...
    case 18:
      value = await ask(
        'Site background image path',
        theme.siteBackgroundImage,
      );
    ...
    case 20:
      value = await ask(
        ...
        theme.headerDescriptionColor || '#555555',
      );
...
function parseDirectArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (arg === '--header-text-color')
      settings.headerTextColor = args[++i];
    else if (arg === '--header-desc-color')
      settings.headerDescriptionColor = args[++i];
    ...
    else if (arg === '--admin-root-help-en')
      settings.adminRootHelpEn = args[++i];
    else if (arg === '--admin-root-help-zh')
      settings.adminRootHelpZh = args[++i];
    else if (arg === '--admin-asset-hint-en')
      settings.adminAssetHintEn = args[++i];
    else if (arg === '--admin-asset-hint-zh')
      settings.adminAssetHintZh = args[++i];
...
function applyDirectChanges(settings, direct) {
  ...
  if (direct.bodyBackgroundImage !== undefined)
    settings.theme.bodyBackgroundImage = direct.bodyBackgroundImage;
  if (direct.headerBackgroundImage !== undefined)
    settings.theme.headerBackgroundImage = direct.headerBackgroundImage;
  if (direct.siteBackgroundImage !== undefined)
    settings.theme.siteBackgroundImage = direct.siteBackgroundImage;
  if (direct.headerTextColor !== undefined)
    settings.theme.headerTextColor = direct.headerTextColor;
  if (direct.headerDescriptionColor !== undefined)
    settings.theme.headerDescriptionColor = direct.headerDescriptionColor;
  if (direct.headerMinHeight !== undefined)
    settings.theme.headerMinHeight = direct.headerMinHeight;
  if (direct.fontFamily !== undefined)
    settings.theme.typography.fontFamily = direct.fontFamily;
  if (direct.baseFontSize !== undefined)
    settings.theme.typography.baseFontSize = direct.baseFontSize;
  if (direct.lineHeight !== undefined)
    settings.theme.typography.lineHeight = direct.lineHeight;
  if (direct.headingFontFamily !== undefined)
    settings.theme.typography.headingFontFamily = direct.headingFontFamily;
  if (direct.headingFontWeight !== undefined)
    settings.theme.typography.headingFontWeight = direct.headingFontWeight;
  if (direct.codeFontFamily !== undefined)
    settings.theme.typography.codeFontFamily = direct.codeFontFamily;
  if (direct.titleEn !== undefined)
    settings.copy.en.site.title = direct.titleEn;
  if (direct.descEn !== undefined)
    settings.copy.en.site.description = direct.descEn;
  if (direct.footerEn !== undefined)
    settings.copy.en.site.footer = direct.footerEn;
  if (direct.titleZh !== undefined && settings.copy['zh-CN'])
    settings.copy['zh-CN'].site.title = direct.titleZh;
  if (direct.descZh !== undefined && settings.copy['zh-CN'])
    settings.copy['zh-CN'].site.description = direct.descZh;
  if (direct.footerZh !== undefined && settings.copy['zh-CN'])
    settings.copy['zh-CN'].site.footer = direct.footerZh;
  if (direct.noticeEn !== undefined)
    settings.copy.en.home.notice = direct.noticeEn;
  if (direct.noticeZh !== undefined && settings.copy['zh-CN'])
    settings.copy['zh-CN'].home.notice = direct.noticeZh;
  if (direct.sidebarTitleEn !== undefined)
    settings.copy.en.sidebar.aboutTitle = direct.sidebarTitleEn;
  if (direct.sidebarTitleZh !== undefined && settings.copy['zh-CN'])
    settings.copy['zh-CN'].sidebar.aboutTitle = direct.sidebarTitleZh;
  if (direct.sidebarTextEn !== undefined)
    settings.copy.en.sidebar.aboutText = direct.sidebarTextEn;
  if (direct.sidebarTextZh !== undefined && settings.copy['zh-CN'])
    settings.copy['zh-CN'].sidebar.aboutText = direct.sidebarTextZh;
  if (direct.adminIntroEn !== undefined)
    settings.copy.en.admin.intro = direct.adminIntroEn;
  if (direct.adminIntroZh !== undefined && settings.copy['zh-CN'])
    settings.copy['zh-CN'].admin.intro = direct.adminIntroZh;
  if (direct.adminRootHelpEn !== undefined)
    settings.copy.en.admin.rootHelp = direct.adminRootHelpEn;
  if (direct.adminRootHelpZh !== undefined && settings.copy['zh-CN'])
    settings.copy['zh-CN'].admin.rootHelp = direct.adminRootHelpZh;
  if (direct.adminAssetHintEn !== undefined)
    settings.copy.en.admin.assetHintBeforePost = direct.adminAssetHintEn;
  if (direct.adminAssetHintZh !== undefined && settings.copy['zh-CN'])
    settings.copy['zh-CN'].admin.assetHintBeforePost = direct.adminAssetHintZh;
}
